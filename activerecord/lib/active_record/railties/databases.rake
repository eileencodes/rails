...
db_namespace = namespace :db do
  ...
  task migrate: :load_config do
    ActiveRecord::Base.configurations(legacy: false).configs_for(Rails.env).each do |db_config|
  ...
  namespace :test do
    ...
    task load_schema: %w(db:test:purge) do
      begin
        ...
        ActiveRecord::Base.configurations(legacy: false).configs_for("test").each do |db_config|
          filename = ActiveRecord::Tasks::DatabaseTasks.dump_filename(db_config.spec_name, :ruby)
          ActiveRecord::Tasks::DatabaseTasks.load_schema(db_config.config, :ruby, filename, "test")
        end
      ensure
        if should_reconnect
          ActiveRecord::Base.establish_connection(ActiveRecord::Base.configurations(legacy: false).default_config_hash(ActiveRecord::Tasks::DatabaseTasks.env))
        end
      end
    end
