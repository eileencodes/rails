  ...
  module RakeTests
    class RakeDbsTest < ActiveSupport::TestCase
      ...
      test "db:create and db:drop without database url" do
        ...
        db_create_and_drop ActiveRecord::Base.configurations(legacy: false).default_config(Rails.env)["database"]
      end
      ...
      test "db:migrate and db:migrate:status without database_url" do
        require "#{app_path}/config/environment"
        db_migrate_and_status ActiveRecord::Base.configurations(legacy: false).default_config(Rails.env)["database"]
      end
      ...
      test "db:fixtures:load without database_url" do
        require "#{app_path}/config/environment"
        db_fixtures_load ActiveRecord::Base.configurations(legacy: false).default_config(Rails.env)["database"]
      end
      ...
      test "db:structure:dump and db:structure:load without database_url" do
        require "#{app_path}/config/environment"
        db_structure_dump_and_load ActiveRecord::Base.configurations(legacy: false).default_config(Rails.env)["database"]
      end
      ...
      def db_test_load_structure
        Dir.chdir(app_path) do
          ...
          # if structure is not loaded correctly, exception would be raised
          assert_equal 0, Book.count
          assert_match ActiveRecord::Base.configurations(legacy: false).default_config("test")["database"],
